...
global {
	...
	action update_taxi_population (int new_number) {
		...
		if (delta > 0) {
			...
			create taxi_ev number: -delta with: [all_stations::all_stations, use_chargerbot_dispatch::use_chargerbot_dispatch, location::one_of(buildings_type_1).location];
	...
	action update_private_population (int new_number) {
		...
		if (delta > 0) {
			...
			create private_ev number: -delta with: (all_stations::all_stations, use_chargerbot_dispatch::use_chargerbot_dispatch, location::one_of(buildings_type_1).location);
		}
	}

	action update_rush_private_population (int new_number) {
		create rush_hour_private number: new_number with: (all_stations::all_stations, use_chargerbot_dispatch::use_chargerbot_dispatch, location::one_of(buildings_type_1).location);
	}

	action update_rush_taxi_population (int new_number) {
		create rush_hour_taxi number: new_number with: (all_stations::all_stations, use_chargerbot_dispatch::use_chargerbot_dispatch, location::one_of(buildings_type_1).location);
	}
