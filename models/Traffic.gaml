...
species chargerbot parent: vehicle skills: [network, driving] {
	...
	int   charge_rate      <- 0;     // kW rate của cổng đang chiếm khi tự sạc
	float waiting_time     <- 0.0;   // tổng thời gian chờ cổng ở station (phút sim)
	float max_speed <- ele_car_speed #km / #h;
	...
	list<charging_station> all_stations <- []; // Truyền vào lúc create trong Main.gaml (chargerbot không kế thừa elecar nên không có sẵn)
...
species elecar parent: vehicle {
	...
	bool use_chargerbot_dispatch <- true;    // Truyền vào lúc create trong Main.gaml (global cùng tên không tự thấy được từ species)
